...
const C = 'pb_cache_', L = 'pb_'

window.storage = {
  async get(key, shared) {
    if (!shared) { const v = localStorage.getItem(L+key); if(v==null) throw new Error('not found'); return {key,value:v} }
    try {
      const {data,error} = await sb.from('kv_store').select('value').eq('key',key).limit(1)
  ...
  async set(key, value, shared) {
    if(!shared) { localStorage.setItem(L+key,value); return {key,value} }
    localStorage.setItem(C+key, value)
    try {
      const {error} = await sb.from('kv_store').upsert({key,value,updated_at:new Date().toISOString()},{onConflict:'key'})
      if(error) throw error
  ...
  async delete(key,shared) {
    ...
    if(shared) await sb.from('kv_store').delete().eq('key',key).catch(()=>{})
    return {key,deleted:true}
  },
  async list(prefix,shared) {
    const pfx=(shared?C:L)+(prefix||'')
    return {keys:Object.keys(localStorage).filter(k=>k.startsWith(pfx)).map(k=>k.replace(shared?C:L,''))}
  }
}
